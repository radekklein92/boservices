...
// Export podkladu pro účetní (XLSX). Sekce je defaultně sbalená (ať nezabírá
// místo) - rozbalí se klikem na nadpis se šipkou. Filtr na stav výběru +
// období (dle data podkladu); prázdný filtr = všechno. Stahuje přes fetch+blob
// kvůli ošetření chyb a názvu souboru z hlavičky. Jen pro adminy (renderuje se
// podmíněně).
export function CommissionsExportClient() {
  const [open, setOpen] = useState(false); // sbaleno
  ...
  return (
    <section>
      {/* Klikací nadpis se šipkou - rozbalí/sbalí panel exportu */}
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
      ...
      >
        <ChevronDown
          className={`h-4 w-4 shrink-0 text-ink-soft transition-transform group-hover:text-ink-mid ${open ? "" : "-rotate-90"}`}
          strokeWidth={1.5}
          aria-hidden="true"
        />
