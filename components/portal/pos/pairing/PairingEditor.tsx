...
// (lokalita portálu). Návrh = nejlépe sedící volná lokalita dle podobnosti názvů
// (počítá server). Zápis přes /api/portal/pos/pairing (drží 1 lokalita <-> 1 pokladna).
export function PairingEditor({
  ...
  const assignedLoc = useMemo(() => {
    const set = new Set<string>();
    for (const p of Object.values(pairs)) if (p.locationId) set.add(p.locationId);
    return set;
  }, [pairs]);
  const unpairedLocations = useMemo(() => locations.filter((l) => !assignedLoc.has(l.id)), [locations, assignedLoc]);
    ...
    <section className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <span className="text-[12px] tabular-nums text-ink-mid">
          {pairedCount} s prodejnou · {shops.length - pairedCount} bez
        </span>
        <input
          type="search"
          value={search}
        ...
        <div className="inline-flex rounded-lg border border-edge p-0.5">
          {(["all", "unpaired", "paired"] as StatusFilter[]).map((st) => (
            ...
            >
              {st === "all" ? "Vše" : st === "unpaired" ? "Nenapárované" : "Napárované"}
            </button>
          ))}
        </div>
      </div>
              ...
              {editingThis && (
                <div className="flex flex-wrap items-end gap-3 border-t border-edge/60 bg-paper-warm px-4 py-3">
                  <label className="flex min-w-[240px] flex-1 flex-col gap-1">
                    ...
                    >
                      <option value="">— Bez prodejny —</option>
                      {locations.map((l) => {
                        const taken = assignedLoc.has(l.id) && l.id !== cur?.locationId;
                        return (
