...
import {
  DEBTOR_PRESETS as COMPANY_PRESETS,
  type DebtorPreset as CompanyPreset,
} from "@/lib/portal/debtor-presets";

// Společné rozhraní pro vyplnění firmy: dlužník, manažer, poskytovatel atd.
// Konkrétní typ pole (debtorName vs. managerName...) namapuje rodičovský komponent.
export interface CompanyFillPayload {
  name: string;
  ico: string;
  street: string;
  city: string;
...
export function CompanyChipPicker({
  selectedIco,
  onFill,
  addLabel = "Jiná firma",
  modalTitle = "Vyhledat firmu v ARES",
  modalEyebrow = "Firma mimo presety",
}: {
  selectedIco?: string;
  onFill: (payload: CompanyFillPayload) => void;
  addLabel?: string;
  modalTitle?: string;
  modalEyebrow?: string;
}) {
  ...
  async function pick(preset: CompanyPreset) {
...
function AresLookupModal({
  ...
  eyebrow: string;
  onClose: () => void;
  onFilled: (payload: CompanyFillPayload) => void;
}) {
