...
// DigiSign: nad podpisový blok BOS i klienta vstříkne neviditelnou DigiSign kotvu
...
//
// Strana se rozezná podle párty-labelu (statický text v šabloně, funguje i po
// dosazení proměnných): BOS = "za Poskytující stranu/Poskytovatele/Manažera:",
// klient = "za Přijímající stranu/Příjemce/Franšízanta:". Pokrývá NDA i
// franchise/cooperation/operation. Když label v šabloně chybí (ruční úprava),
// kotva se nevloží a DigiSign použije vypočtené pozice (computeSignaturePositions).
const SIG_GAP = '<div style="height:84px"></div>';
const signatureAnchor = (id: string) =>
  `<span style="font-size:2px;color:transparent">${id}</span>`;

const BOS_SIGNATURE_RE =
  /<p>((?:(?!<\/p>)[\s\S])*?za (?:Poskytující stranu|Poskytovatele|Manažera):(?:(?!<\/p>)[\s\S])*?)<\/p>/;
const CLIENT_SIGNATURE_RE =
  /<p>((?:(?!<\/p>)[\s\S])*?za (?:Přijímající stranu|Příjemce|Franšízanta):(?:(?!<\/p>)[\s\S])*?)<\/p>/;

export function wrapSignatureUnits(html: string): string {
  let out = html.replace(
    BOS_SIGNATURE_RE,
    `<div class="sign-unit">${SIG_GAP}<p>${signatureAnchor("signBosFld")}$1</p></div>`,
  );
  out = out.replace(
    CLIENT_SIGNATURE_RE,
    `<div class="sign-unit">${SIG_GAP}<p>${signatureAnchor("signClientFld")}$1</p></div>`,
  );
  return out;
}

// Zpětně kompatibilní alias (NDA volání). Chování shodné - generická funkce
// pokrývá i párty-labely NDA.
export function wrapNdaSignatureUnits(html: string): string {
  return wrapSignatureUnits(html);
}
