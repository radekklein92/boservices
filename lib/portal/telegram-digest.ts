...
export async function runTelegramLocationDigest(
  ...
  for (const agent of configured) {
    ...
    //  - NEVYŘEŠENÉ červené (recon needs) jsou samostatná kategorie → jdou sem jen
    //    s příznakem "stejně řešit" (solveDespiteRed); červená = z importu
    //    (flaggedRed) NEBO ruční (manualRed), ať se chovají stejně jako v tabulce;
    //    vyřešená červená přepadla do Vyřešeno a sem nepatří,
    ...
    const attention = locations.filter((l) => {
      ...
      const isRed = Boolean(local.newco.flaggedRed) || Boolean(local.manualRed);
      if (isRed) return needsLease && Boolean(local.solveDespiteRed);
      return needsLease;
    });
